...
allTime = tic;
try
    load('CNNPermGeneral.mat');%加载CNN
catch
    error('CNN could not be loaded');
end
CNNHardware = 'gpu'; % 设置计算硬件'gpu'或'cpu'
...
molarVolume{1} = 39.63 * 1e-3; % first index refers to calcite, second to
...
MassOfD = NaN(numberOfSlices, numel(timeSteps)); % mass of different minerals present [mol]
...
[outX, outY] = GetSpeRectangles(220/numberOfSlicesX); %obtain initial rectangle sizes
...
[XCNN, YCNN] = meshgrid(linspace(-0.5, 0.5, 64));%将几何演化网格插值到CNN网格

coarseLevel = zeros(numPartitionsMicroscaleCoarse+1, numPartitionsMicroscaleCoarse+1, numberOfSlices);
CNNLevel = zeros(64, 64, 1, 2*numberOfSlices);%包含x和y两个方向上的主轴渗透率
tic
...
tic
for (i = 1:numberOfSlices)
    CNNLevel(:, :, 1, i) = int8(interp2(X, Y, reshape(microCells.signed{i}(:, 1), numPartitionsMicroscale + 1, numPartitionsMicroscale + 1), XCNN, YCNN) > 0);
end
toc
CNNLevel(:, :, 1, (numberOfSlices + 1):2*numberOfSlices) = turn(CNNLevel (:, :, 1, 1:numberOfSlices), -pi/2);%旋转90度，以包含y方向的渗透率
CNN = exp(10*(net.predict(CNNLevel, 'ExecutionEnvironment', CNNHardware) - 0.9)) * (meanGrainSize^2);%CNN核心计算过程

for i = 1:numberOfSlices
    permeabilityTensors{i}(1, 1) = CNN(i);
    permeabilityTensors{i}(2:3, 1) = 0;
    permeabilityTensors{i}(4, 1) = CNN(i+numberOfSlices);
end
...
while transportStepper.next
    ...
    %Average concentrations over macroscopic subdomains associated with each
    %unit cell
    ...
    for i = 1:numberOfSlices
        ...
        %normalSpeed_{i,j} denotes normal speed of interface seperating
        %phase i and j
    ...
    index1 = (microRunSinceUpdate > 0.2 / numPartitionsMicroscaleCoarse);
    index2 = find(index1-1); %indizes of unit cells not to be updated
    index1 = find(index1); %indizes of unit cells to be updated
    saveCellAdap{timeIterationStep} = index1;

    %local unpacking variables
    ...
    tic
    for ind = 1:numel(index1) %, numProc)
        i = index1(ind);
        help7(:, :, 1, ind) = int8(interp2(X, Y, reshape(microCells.signed{i}(:, microCells.memoryStep), numPartitionsMicroscale + 1, numPartitionsMicroscale + 1), XCNN, YCNN) > 0);
    end

    %采用CNN进行渗透率预测
    if numel(index1) > 0
        help7(:, :, 1, (numel(index1) + 1):2*numel(index1)) = turn(help7(:, :, 1, 1:numel(index1)), -pi/2);%旋转
        CNN = exp(10*(net.predict(help7, 'ExecutionEnvironment', CNNHardware) - 0.9));%计算

        help4(:, 1) = CNN(1:numel(index1)) * (meanGrainSize^2);
        help4(:, 4) = CNN((numel(index1)+1):2*numel(index1)) * (meanGrainSize^2);
    end
    timePerm = timePerm + toc;
    ...
    disp([num2str(Adapcount), ' recalculation(s) saved by adaptivity :-)']);
