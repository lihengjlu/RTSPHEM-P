...
function microCell = evolveStepGeneral(microCell, interfaceVelocities, ...
...
parfor index = 1:numToCompute  
        ...
        for j = 1:ceil(T/CFL)
            ...
            [dV, S] = step234General(distFunctions{index}, cellGrid, Xi{index}, interfaceVelocities{index}, DistanceDV);%注意:step234 > step234General
            ...
            [distFunctions{index}, Xi{index}] = step1General(Xi{index}, Phi{index}, cellGrid, restrictDist);%注意：step1 > step1General

        end

        [interfaceLengthTemp, ~] = evaluateInterfaceTest(cellGrid, Xi{index}, distFunctions{index}, true);%注意:evaluateInterfaceTe > evaluateInterfaceTest
	     interfaceLength{index} = interfaceLengthTemp(1:5);%注意:2 > 5
        ...
        LevTemp(Xi{index} == 4) = max(LevTemp(Xi{index} == 4), 10^(-4));
        LevTemp(Xi{index} == 5) = max(LevTemp(Xi{index} == 5), 10^(-4));
        LevTemp(Xi{index} == 6) = max(LevTemp(Xi{index} == 6), 10^(-4));
        Lev{index} = LevTemp;
